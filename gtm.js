...
return null},Zc=!1;if(F.querySelectorAll)try{var $c=F.querySelectorAll(":root");$c&&1==$c.length&&$c[0]==F.documentElement&&(Zc=!0)}catch(a){}var Wc=Zc;var K={},od=null,pd=Math.random();K.s="GTM-TZZBRK5";K.Ob="6o0";K.Ld="";var qd={__cl:!0,__ecl:!0,__ehl:!0,__evl:!0,__fal:!0,__fil:!0,__fsl:!0,__hl:!0,__jel:!0,__lcl:!0,__sdl:!0,__tl:!0,__ytl:!0,__paused:!0,__tg:!0},rd="www.googletagmanager.com/gtm.js";
var sd=rd,td=null,ud=null,vd=null,wd="//www.googletagmanager.com/a?id="+K.s+"&cv=22",xd={},yd={},zd=function(){var a=od.sequence||0;od.sequence=a+1;return a};
...
function f(w){G(function(){function y(){for(var A=x.getElementsByTagName("iframe"),B=A.length,z=0;z<B;z++)e(A[z],w)}var x=W("document");y();mk(y)})}function h(w,y){var x,A;function B(){fa=Gk(function(){return{url:V,title:aa,oe:J,ng:w.getCurrentTime(),playbackRate:X}},y.ud,w.getIframe());J=0;aa=V="";X=1;return z}function z(ma){switch(ma){case q.PLAYING:J=Math.round(w.getDuration());V=w.getVideoUrl();if(w.getVideoData){var P=w.getVideoData();aa=P?P.title:""}X=w.getPlaybackRate();y.hg?fa.Ub("start"):
fa.Nc();M=m(y.dh,y.bh,w.getDuration());return D(ma);default:return z}}function D(){va=w.getCurrentTime();ta=(new Date).getTime();fa.lh();la();return H}function H(ma){var P;switch(ma){case q.ENDED:return N(ma);case q.PAUSED:P="pause";case q.BUFFERING:var R=w.getCurrentTime()-va;P=1<Math.abs(((new Date).getTime()-ta)/1E3*X-R)?"seek":P||"buffering";w.getCurrentTime()&&(y.gg?fa.Ub(P):fa.Nc());O();return L;case q.UNSTARTED:return B(ma);default:return H}}function L(ma){switch(ma){case q.ENDED:return N(ma);
case q.PLAYING:return D(ma);case q.UNSTARTED:return B(ma);default:return L}}function N(){for(;A;){var ma=x;Hk(A);ma()}y.fg&&fa.Ub("complete",1);return B(q.UNSTARTED)}function Q(){}function O(){A&&(Hk(A),A=0,x=Q)}function la(){if(M.length&&0!==X){var ma=-1,P;do{P=M[0];if(P.Da>w.getDuration())return;ma=(P.Da-w.getCurrentTime())/X;if(0>ma&&(M.shift(),0===M.length))return}while(0>ma);x=function(){A=0;x=Q;0<M.length&&M[0].Da===P.Da&&(M.shift(),fa.Ub("progress",P.Me,P.Te));la()};A=Ik(x,1E3*ma)}}var fa,
M=[],J,V,aa,X,va,ta,Ga=B(q.UNSTARTED);A=0;x=Q;return{onStateChange:function(ma){Ga=Ga(ma)},onPlaybackRateChange:function(ma){va=w.getCurrentTime();ta=(new Date).getTime();fa.Nc();X=ma;O();la()}}}function k(w){for(var y=w.split(","),x=y.length,A=[],B=0;B<x;B++){var z=parseInt(y[B],10);isNaN(z)||100<z||0>z||A.push(z/100)}A.sort(function(D,H){return D-H});return A}function l(w){for(var y=w.split(","),x=y.length,A=[],B=0;B<x;B++){var z=parseInt(y[B],10);isNaN(z)||0>z||A.push(z)}A.sort(function(D,H){return D-
...
if(y||x||A||B.length||z.length){var H={hg:y,fg:x,gg:A,bh:B,dh:z,ve:D,ud:void 0===w.vtp_uniqueTriggerId?"":w.vtp_uniqueTriggerId},L=W("YT"),N=function(){f(H)};G(w.vtp_gtmOnSuccess);if(L)L.ready&&L.ready(N);else{var Q=W("onYouTubeIframeAPIReady");Nk("onYouTubeIframeAPIReady",function(){Q&&Q();N()});G(function(){for(var O=W("document"),la=O.getElementsByTagName("script"),fa=la.length,M=0;M<fa;M++){var J=la[M].getAttribute("src");if(b(J,"iframe_api")||b(J,"player_api"))return}for(var V=O.getElementsByTagName("iframe"),
